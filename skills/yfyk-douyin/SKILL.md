...
5. Generate and show the production template picker before creating `job.json`:
...
Wait for the user to click one of four real previews or `均衡随机`. Never choose a default. For a fixed choice, write that registered template ID to every selected document unless the user requests per-document choices. For `balanced-random`, create a stable seed, call `template_assignment.apply_template_choice`, show the resulting counts, and write every materialized per-document `card_template`. Do not write `balanced-random` as a document template.
